...
// "/home/penny/Desktop/mirlca/tests/fs_credits.txt"
...
/*
		...
		// localhost and the port sclang is listening on
		~ip = "127.0.0.1";
		~port = 57120;
...
*/
        ...
        if(backend == 0){
            ...
            Freesound.token="yaWlrOVH49w8JRPB88KXAoIOUXaFSTiNKHmVsdI6"; // change it to own API key token
			} // end try
...
/*		SynthDef(\limit, {
    ...
    tag { |tag = "noise", size = 1|
        ...
        backendClass.textSearch(query: tag, params: ('page': 1),
            action: { |p|
				...
				~pypath = (~path.asString +/+ "python/"); //spesific path for the file that python will read
				File.mkdir(~pypath); //create the path if it does not exist
				//(PathName( ~path.asString +/+ "python/")).postln
				~f = File( (~pypath.asString+/+"soundnames_analysis.txt").standardizePath, "a" );
	...
	melody { |shape = 'straight', tag = "noise", range = 4, size = 1|
		...
		backendClass.textSearch(query: tag, params: ('page': 1),
			action: { |p|
				...
				//---------------
				~path = "./mirlc2/"; //general path
				~pypath = (~path.asString +/+ "python/"); //spesific path for the file that python will read
				File.mkdir(~pypath); //create the path if it does not exist
				//(PathName( ~path.asString +/+ "python/")).postln
				~f1 = File( (~pypath.asString+/+"soundnames_analysis.txt").standardizePath, "w" ); // "a" for kepping all
				~f2 = File( (~pypath.asString+/+"shape.txt").standardizePath, "w" );
				...
				if ( p.isNil,
					{
						postln("ERROR: There was a problem downloading this sound using this method.\nINFO: Please try again.");
					}, {

						size.do { |index|
							snd = p[index];
							postln("INFO: Found sound by tag, name: " ++ snd["name"]);
							while (
								{this.sndidexist(snd.id) == 1},
								{
									postln ("INFO: Repeated sound, getting another sound...");
							...
							//-----------------
							~f1.write(snd["name"].asString ++ " \n");
							~f2.write(shape.asString ++ " \n" ++ " \n" ++ range ++ " \n");
							//shape.asString.postln;
							//postln (snd["name"].asString ++ " POST");
							//-----------------
		...
		// call python script
		~a = "/mnt/DATA/thesis/code/mirlc2/python/soundnames_analysis.txt";
		~b = "/mnt/DATA/thesis/code/mirlc2/sounds";
		~return = ("python3 /mnt/DATA/thesis/pycode/main_script.py " ++ ~a.asString ++ " " ++ ~b.asString).unixCmdGetStdOut;
		...
		//load it to a buffer
		~buf = Buffer.read(server, ~return.asString);

		~buf.play; //πρεπει να ειναι χωριστα αλλιώς δεν τρέχει
		//μηπως δεν ειναι καλο να στέλνω το path? γτ δεν ξέρουμε ποιο id ειναι....

		//this.id(snd.id, 1); //loaded & directly played
